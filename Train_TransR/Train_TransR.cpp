...
public:
	...
	void run(int n_in, double rate_in, double margin_in, int method_in)
	{
		...
		FILE* f1 = fopen(("../TransE/entity2vec." + dim_str + "." + version).c_str(), "r");
		...
		FILE* f2 = fopen(("../TransE/relation2vec." + dim_str + "." + version).c_str(), "r");
		for (int i = 0; i<relation_num; i++)
		{
			for (int ii = 0; ii<n; ii++)
...
private:
	...
	void bfgs()
	{
		...
		for (int epoch = 0; epoch<nepoch; epoch++)
		{
			...
			FILE* f1 = fopen(("relation2vec." + dim_str + "." + version).c_str(), "w");
			FILE* f2 = fopen(("entity2vec." + dim_str + "." + version).c_str(), "w");
			FILE* f3 = fopen(("A." + dim_str + "." + version).c_str(), "w");
			for (int i = 0; i<relation_num; i++)
			{
				for (int ii = 0; ii<m; ii++)
